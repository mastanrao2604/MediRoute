"""Pincode fields + ongoing multi-nurse staff search columns.
...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE shift_requests ADD COLUMN IF NOT EXISTS nurses_required "
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE shift_requests DROP COLUMN IF EXISTS search_closed_at"
    )
    op.execute(
        "ALTER TABLE shift_requests DROP COLUMN IF EXISTS nurses_required"
    )
    op.execute(
        "ALTER TABLE shift_requests DROP COLUMN IF EXISTS hospital_locality"
    )
    op.execute(
        "ALTER TABLE profiles DROP COLUMN IF EXISTS location_source"
    )
    op.execute(
        "ALTER TABLE profiles DROP COLUMN IF EXISTS service_locality"
    )
    op.execute(
        "ALTER TABLE profiles DROP COLUMN IF EXISTS service_pincode"
    )
    op.execute(
        "ALTER TABLE shift_requests DROP COLUMN IF EXISTS hospital_pincode"
    )
