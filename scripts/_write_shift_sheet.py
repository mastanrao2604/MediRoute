...
export default function RecruiterShiftDetailSheet({{
  ...
  return (
    <{T} className="fixed inset-0 z-50 flex flex-col justify-end bg-black/40" role="dialog" aria-modal="true">
      <button type="button" className="absolute inset-0" aria-label="Close" onClick={{onClose}} />
      <{T} className="relative bg-white rounded-t-2xl shadow-xl max-h-[90vh] overflow-y-auto pb-[env(safe-area-inset-bottom)]">
        <{T} className="sticky top-0 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between z-10">
        ...
        <{T} className="px-4 py-4 pb-8">
          {{loading && <{T} className="flex justify-center py-8"><Spinner /></{T}>}}
          ...
          {{shift && !loading && (
            <>
              ...
              {{editable && form ? (
                ...
                <{T} className="text-sm text-gray-600 space-y-2 mb-4">
              ...
              <{T} className="flex flex-col gap-2 mt-4 pt-4 border-t border-gray-100">
...
Path(r"d:\MediRoute\frontend\src\components\recruiter\RecruiterShiftDetailSheet.jsx").write_text(content, encoding="utf-8")
print("written")
