...
.SYNOPSIS
    ...
    Run this ONCE on a new machine. It installs all dependencies,
    scaffolds your .env files, and opens them for you to fill in.
...
$javaFound = (Get-Command java -ErrorAction SilentlyContinue) -or
             ($env:JAVA_HOME -and (Test-Path (Join-Path $env:JAVA_HOME "bin\java.exe")))
if (-not $javaFound) {
    Warn "Java not found. Install Android Studio (it includes JDK) from https://developer.android.com/studio"
...
Write-Host ""
Write-Host "  mediroute-backend/.env — fill in:" -ForegroundColor White
...
Write-Host ""
Write-Host "  frontend/.env — fill in:" -ForegroundColor White
...
Write-Host "  Fill in the values, save both files, then press Enter to continue." -ForegroundColor Cyan
Write-Host ""
Read-Host "  Press Enter when .env files are saved"
...
Write-Host ""
Write-Host "  Connect your Android device via USB (USB Debugging ON)" -ForegroundColor White
